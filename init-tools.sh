...
if [ ! -e $__INIT_TOOLS_DONE_MARKER ]; then
    ...
    if [ ! -e $__DOTNET_PATH ]; then
        ...
        if [ -n "$DOTNET_TOOLSET_DIR" ] && [ -d "$DOTNET_TOOLSET_DIR/$__DOTNET_TOOLS_VERSION" ]; then
            ...
            __DOTNET_LOCATION="https://dotnetcli.blob.core.windows.net/dotnet/preview/Binaries/Latest/dotnet-dev-ubuntu-x64.latest.tar.gz"
            ...
            cp "$__scriptpath/libcoreclr.so" shared/Microsoft.NETCore.App/1.0.0/libcoreclr.so
